...
class _EditDreamState extends State<EditDream> {
  ...
  @override
  void initState() {
    editDream = Get.arguments;
    _dreamApi = _futureBuild();
    _futureEditDream();
    super.initState();
  }

  void _futureEditDream() {
    _nameController.text = editDream!['name'];
    _categoryVal = editDream!['category_id'] != null ? editDream!['category_id'].toString() : null;
    _categorySubVal = editDream!['subCategory_id'] != null ? editDream!['subCategory_id'].toString() : null;
    _dateController.text = editDream!['targetDate'];
  }
  ...
  Widget build(BuildContext context) {
    var dreamList = editDream;
    return Scaffold(
      backgroundColor: Colors.white,
      key: this._scaffoldKey,
      appBar: AppBar(
        title: Text(Translator.get('Edit Dream')!),
        centerTitle: true,
        elevation: 0,
      ),
      body: FutureBuilder(
              ...
              GestureDetector(
                onTap: () async {
                  ...
                  if (_uploadingDetailsKey.currentState!.validate()) {
                    ...
                    Api.http.post('update-dream', data: sendData).then(
                      (response) {
                        GetBar(
  ...
  Widget _buildDreamNameField(BuildContext context) {
    return TextFormField(
      ...
      validator: (value) => value!.isEmpty ? Translator.get("Name can't be empty") : null,
      decoration: InputDecoration(
        border: UnderlineInputBorder(),
          ...
          child: Column(
            ...
            children: <Widget>[
              TextFormField(
                ...
                decoration: InputDecoration(
                  border: UnderlineInputBorder(),
                  labelText: Translator.get('Image Name'),
                  suffixIcon: Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
